...
AS 
BEGIN 
    SELECT col.collectionId ,title, author, bookstatus, COUNT(col.collectionId) number
    FROM Collection col JOIN Book bok ON col.collectionId = bok.collectionId
    JOIN BookStatus bokstu ON bok.bookStatusId = bokstu.bookStatusId
    LEFT JOIN Reservation re ON bok.bookId = re.bookId
...
BEGIN 
    BEGIN TRY
        ...
        BEGIN
            ...
            BEGIN 
                SELECT 2 AS ResultCode, ('此借閱者並不是可以借書的借閱者!') AS Message;
                ROLLBACK
            END
