...
BEGIN
		...
		JOIN Author auth ON auth.authorId = col.collectionId
...
BEGIN 
    BEGIN TRY
        ...
        BEGIN
            SELECT 0  ResultCode, ('該借閱者不存在，請重新輸入!') AS Message;
            ROLLBACK
            RETURN
        END
        ...
        BEGIN
            ROLLBACK
            SELECT 0   ResultCode, ('該本書籍不存在，請重新輸入!') AS Message;
            RETURN
        END

        DECLARE @bookid INT,@collectionId INT
		SELECT @bookid = bookid, @collectionId = collectionId
            FROM Book
        ...
        BEGIN
				...
				BEGIN 
					SELECT 0 ResultCode, '不能重複借閱同一本書籍' Message
					RETURN
				END
			-- 直接更新為借出狀態(1)
            ...
            IF @@ROWCOUNT = 0
            BEGIN
                SELECT 0  ResultCode, '該本書已被其他人借走' Message;
                ROLLBACK;
                RETURN;
            END
			-- 新增借閱紀錄
            ...
            SELECT 1 AS ResultCode, ('借閱成功') AS Message;
            COMMIT
            RETURN
        END
        -- 如果有在網路上預約(2)的話
        ...
        BEGIN
            ...
            BEGIN 
                SELECT 0 AS ResultCode, ('此書本已被借閱!') AS Message;
                ROLLBACK
				RETURN
            END
            ...
            IF @@ROWCOUNT = 0
            BEGIN 
                SELECT 1  ResultCode, ('書籍狀態已變更，無法完成借書') AS Message;
                ROLLBACK;
                RETURN;
            END
        ...
        END
        -- 
        SELECT 0 ResultCode, ('該本書無法借閱。') AS Message;
        ROLLBACK
        RETURN
    END TRY
    BEGIN CATCH 
...
BEGIN
    BEGIN TRY
		...
		IF @ResultCode = 0
		BEGIN 
				SELECT 0 ResultCode, '通知預約者失敗'  Message
				RETURN
		END
...
-- 書籍查詢
CREATE FUNCTION BookQueryResultView()
RETURNS TABLE
AS
RETURN(
	SELECT col.collectionId, col.isbn ,col.collectionImg , title, auth.author, col.collectionDesc, col.publisher, col.publishDate, ISNULL(COUNT(*), 0) NumberOfBook
	FROM Collection col 
	JOIN Author auth ON col.authorId = auth.authorId
	LEFT JOIN Book bok ON col.collectionId = bok.collectionId
	GROUP BY col.collectionId,col.isbn, collectionImg, title, auth.author,col.collectionDesc, col.publisher, col.publishDate
)
GO